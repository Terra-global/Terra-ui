...
import { analyzeAnimal, type FactSheet } from '@terra-oracle/terra-oracle';
...
export default function AnimalPage() {
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await analyzeAnimal(subject, manualCoords, {
  ...
  return (
    <div className="animate-fadeIn">
      <header style={{ marginBottom: '2rem', textAlign: 'center' }}>
        <h1 style={{ fontSize: '1.5rem', color: '#b91c1c' }}>Livestock Analysis</h1>
        <p style={{ fontSize: '0.9rem', color: '#64748b' }}>Poultry & Animal heatwave tracking</p>
      </header>
        ...
        <div style={{ padding: '1.25rem', borderRadius: '1.25rem', background: 'white', border: '1px solid #e2e8f0' }}>
          <form onSubmit={handleLookup} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
                <input
                    type="text"
                    placeholder="Enter animal type (e.g., Poultry)..."
                    value={subject}
                    onChange={(e) => setSubject(e.target.value)}
                    disabled={loading}
                    style={{ flex: 1, padding: '0.75rem 1rem', borderRadius: '0.75rem', border: '1px solid #e2e8f0', fontSize: '0.9rem' }}
                />
                <button type="submit" disabled={loading} style={{ background: '#b91c1c', color: 'white', padding: '0 1.25rem', borderRadius: '0.75rem', border: 'none', cursor: 'pointer' }}>
          ...
          {aiAdvice && (
            <div style={{ padding: '1.5rem', background: 'white', borderRadius: '1.25rem', border: '1px solid #e2e8f0' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#10b981', marginBottom: '1rem' }}>
                <Sparkles size={16} /> <span style={{ fontWeight: 600 }}>Veterinary Advisory</span>
              </div>
